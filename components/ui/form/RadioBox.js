...
export default class UiRadioBox extends React.Component {
  ...
  componentDidMount() { 
    ...
    if(this.props.defaultValue) this.setState({radioBoxActive: this.props.defaultValue})
  }
  ...
  render() {
    ...
    var List = this.props.itemList.map((item,index)=>{
      return (
          <TouchableOpacity onPress={()=>{ this.setValue(item.value) }} style={[styles.radioBoxInner, (index === this.props.itemList.length - 1) ? styles.lastInner : null ]} key={index}>
            <Text style={styles.radioBoxText}>{item.title}</Text>
            {Platform.OS == 'ios' ? 
              <View  style={styles.radioBoxButton}>
                {this.state.radioBoxActive != item.value ? <Image source={radioImage} style={styles.radioBoxImage}/> : <Image source={radioActiveImage} style={styles.radioBoxImage}/> }
              </View> :
              <View  style={styles.radioBoxButton}>
                {this.state.radioBoxActive != item.value ? <Image source={radioImage} style={styles.radioBoxImage}/> : <Image source={radioActiveImage} style={styles.radioBoxImage}/> }
              </View>
