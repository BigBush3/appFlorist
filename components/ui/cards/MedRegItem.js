...
export default class UiMedRegItem extends React.Component {
  ...
  render() {
    ...
    return (
      <TouchableOpacity style={styles.medRegItem} onPress={this.props.regItemPress}>
        {this.props.registrationType == 1 ? <Image style={styles.medRegItemImage} source={telemedIcon} /> : null}
        {this.props.registrationType == 2 ? <Image style={styles.medRegItemImage} source={doctorIcon} /> : null}
        <View style={styles.medRegItemInner}>
          {this.props.regStatus == 1 ? 
            <Text style={[styles.medRegItemStatus, {color: 'rgb(138,149,157)'}]}>Консультация еще не началась</Text> 
          : null}
          {this.props.regStatus == 2 ? 
            <Text style={[styles.medRegItemStatus, {color: 'rgb(23,174,135)'}]}>Консультация началась</Text> 
          : null}
          {this.props.regStatus == 3 ? 
            <Text style={[styles.medRegItemStatus, {color: 'rgb(0,112,201)'}]}>До закрытия чата осталось {this.props.timeStatus}</Text> 
          : null}
          {this.props.regStatus == 4 ? 
            <Text style={[styles.medRegItemStatus, {color: 'rgba(252,63,63,0.87)'}]}>Чат закрыт</Text> 
          : null}
